...
- [Part 4: Power Section](#part-4-power-section)
  ...
  - [Layout Guidelines](#layout-guidelines-1)
    ...
    - [±12V DC/DC module (RS3-1212D)](#12v-dcdc-module-rs3-1212d)
    - [General](#general)
...
With ±5% supply tolerance (conservative; the Rev 2 RS3-1212D is
...
negligible rail sag — well within the RS3-1212D's ±125 mA per-rail
rating.
...
output-stage draw. The ±12V module selection (Recom RS3-1212D) is sized
...
* **Supply traces (±12V):** Route as a pair to the decoupling caps. Avoid long runs
  from the RS3-1212D — place the module close to the analog section or use wide
  traces (≥0.5mm).
...
(RS3-1212D secondary, Part 4) and terminate at AGND.
...
* **Added ±12 V rail protection** (one SMBJ15CA bidirectional TVS per
  rail, placed on the power sheet near the RS3-1212D output —
...
The RS3-1212D is a regulated isolated DC-DC converter: it can source
...
**Both rail TVSs sit on the power sheet, adjacent to the RS3-1212D
...
Barrel jack (J_DC1, 2.1mm)
  ...
  +12V_RAW ─────────┬──────────────────────────────┐
                    │                              │
                    ▼                              ▼
                 U2 (TPS54302 buck)          U1 (RS3-1212D isolated ±12V, 3 W)
...
* **±12V DC/DC module:** Traco **TMA-1212D** (1 W, ±42 mA/rail) → Recom
  **RS3-1212D** (3 W regulated ±2%, ±125 mA/rail). Driven by the Rev 2
  ...
  budget is entirely main-PCB consumers plus a defensive margin. SIP-8 THT,
  21.8 × 9.2 × 11.1 mm. Continuous auto-recovery short-circuit protection;
  rail-side over-voltage handled by the SMBJ15CA TVSs (D2, D3) at the
  module output, not by the module itself. The earlier Traco TMR 3-1222E
  pick was dropped after Traco flagged the TMR 3E line as "not recommended
  for new designs"; the RS3 is currently in active production. Same SIP-8
  pinout numbering as the TMR 3-1222E with one wiring difference — see
  CTRL pin warning below.
...
Total output power ≈ 2 × 12 V × 52 mA = **1.25 W**. RS3-1212D rating 3 W
...
* 1× SMAJ5.0A TVS on +5V → GND, on the daisy_seed sheet next to U3 pin 39
  — populated on every build (kit-mandatory)
...
* Isolation slot under the module: the RS3-1212D has isolated primary
  and secondary; preserve 3 mm creepage between PGND (primary) and AGND
  (secondary) on the PCB.
* **CTRL pin (pin 3) — leave OPEN/NC, do not tie to GND.** Per the
  RS3 datasheet: "There is no allowed low state for this pin." Open/
  high-impedance = converter ON (default). Tying pin 3 to GND is the
  pattern for the older TMR family; on the RS3 it's explicitly forbidden
  by the datasheet. Add a `(no_connect)` flag at pin 3 in the schematic
  to satisfy ERC.
...
3. Confirm RS3-1212D symbol / footprint present in
...
4. Pull a 25 mA/rail test load at the ±12V rail caps; verify rails stay
   within the RS3-1212D's ±2% spec. (Not a user-extension test — daughter-
